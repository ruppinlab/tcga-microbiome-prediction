...
parser.add_argument("--out-dir", type=str, default="figures/td_auc", help="out dir")
    ...
    for filename in filenames:
        if m := re.search(split_results_regex, filename):
            ...
            split_results.append(
                load(
                    "{}/surv/{name}/{name}_split_results.pkl".format(
            ...
            if data_type in ("kraken", "htseq"):
                ...
                split_results.append(
                    load(
                        "{}/surv/{name}/{name}_split_results.pkl".format(
                            model_results_dir, name=cox_model_name
                        )
                    )
                )
            else:
                for new_data_type in ("htseq", "kraken"):
                    ...
                    eset_files.append(
                        "{}/{}_eset.rds".format(args.data_dir, new_dataset_name)
                    )
                    split_results.append(
                        load(
                            "{}/surv/{name}/{name}_split_results.pkl".format(
                                model_results_dir, name=new_model_name
                            )
                        )
                    )
            ...
            abbr_dtype_labels = [
                (
                    ...
                    if l == "Combined"
                        ...
                        "Express"
                        if l == "Expression"
                        else "Microbe" if l == "Microbiome" else l
                    )
                )
                for l in dtype_labels
            ]
            ...
            all_cv_split_idxs = Parallel(n_jobs=args.n_jobs, verbose=args.verbose)(
            ...
            for ridx, _ in enumerate(split_results):
                ...
                for split_idx, (train_idxs, test_idxs) in enumerate(
                    ...
                    if not any(y[time_test_idxs][y_stat]):
                        print(
                            "Split {} has all censored y_test samples "
                            "within y_train times, skipping".format(split_idx)
                        )
                        continue
                    y_pred_idxs = np.where(
                        np.isin(test_idxs, time_test_idxs, assume_unique=True)
                    )[0]
                    y_pred = split_result["scores"]["te"]["y_pred"][y_pred_idxs]
                    time = np.arange(
                        np.min(y[time_test_idxs][y_time]),
                        np.max(y[time_test_idxs][y_time]),
                        time_interval_days,
                    )
                    auc = cumulative_dynamic_auc(
                        y[train_idxs], y[time_test_idxs], y_pred, time
                    )[0]
            ...
            xaxis_tick_base = (
                3 if max(mean_times) > 20 else 2 if max(mean_times) > 10 else 1
            )
            ax.set_title(
                figure_title, loc="left", pad=5, fontdict={"fontsize": title_fontsize}
            )
            ax.add_artist(
                AnchoredText(
                    "{}\n"
            ...
            ax.set_xlabel("Years from diagnosis", fontsize=axis_fontsize, labelpad=5)
            ...
            ax.get_yaxis().set_major_formatter(
                ticker.FixedFormatter(["0.2", "0.4", "0.6", "0.8", "1"])
            )
            ...
            for fmt in args.file_format:
                fig.savefig(
                    "{}/{}_td_auc.{}".format(args.out_dir, model_name, fmt),
                    format=fmt,
                    bbox_inches="tight",
                    # matplotlib GH#15497
