...
model_names = {'kraken': ['tcga_acc_surv_os_kraken_cnet',
                          'tcga_acc_surv_pfi_kraken_cnet',
                          'tcga_cesc_surv_os_kraken_cnet',
                          'tcga_lgg_surv_pfi_kraken_cnet',
                          'tcga_skcm_surv_os_kraken_cnet'],
               'htseq': ['tcga_acc_surv_os_htseq_counts_cnet',
                         'tcga_blca_surv_os_htseq_counts_cnet',
                         'tcga_cesc_surv_os_htseq_counts_cnet',
                         'tcga_hnsc_surv_os_htseq_counts_cnet',
                         'tcga_kirc_surv_os_htseq_counts_cnet',
                         'tcga_kirp_surv_os_htseq_counts_cnet',
                         'tcga_lgg_surv_os_htseq_counts_cnet',
                         'tcga_lihc_surv_os_htseq_counts_cnet',
                         'tcga_luad_surv_os_htseq_counts_cnet',
                         'tcga_meso_surv_os_htseq_counts_cnet',
                         'tcga_pcpg_surv_os_htseq_counts_cnet',
                         'tcga_sarc_surv_os_htseq_counts_cnet',
                         'tcga_skcm_surv_os_htseq_counts_cnet',
                         'tcga_stad_surv_os_htseq_counts_cnet',
                         'tcga_thym_surv_os_htseq_counts_cnet',
                         'tcga_ucec_surv_os_htseq_counts_cnet',
                         'tcga_uvm_surv_os_htseq_counts_cnet',
                         'tcga_acc_surv_pfi_htseq_counts_cnet',
                         'tcga_blca_surv_pfi_htseq_counts_cnet',
                         'tcga_cesc_surv_pfi_htseq_counts_cnet',
                         'tcga_kirc_surv_pfi_htseq_counts_cnet',
                         'tcga_kirp_surv_pfi_htseq_counts_cnet',
                         'tcga_lgg_surv_pfi_htseq_counts_cnet',
                         'tcga_lihc_surv_pfi_htseq_counts_cnet',
                         'tcga_lusc_surv_pfi_htseq_counts_cnet',
                         'tcga_meso_surv_pfi_htseq_counts_cnet',
                         'tcga_paad_surv_pfi_htseq_counts_cnet',
                         'tcga_prad_surv_pfi_htseq_counts_cnet',
                         'tcga_sarc_surv_pfi_htseq_counts_cnet',
                         'tcga_stad_surv_pfi_htseq_counts_cnet',
                         'tcga_ucec_surv_pfi_htseq_counts_cnet',
                         'tcga_uvm_surv_pfi_htseq_counts_cnet']}

parser = ArgumentParser()
parser.add_argument('--data-type', type=str, default='kraken',
                    choices=['kraken', 'htseq'],
                    help='data type')
...
for midx, model_name in enumerate(model_names):
    ...
    split_results = []
    split_results.append(load('{}/surv/{name}/{name}_split_results.pkl'
    ...
    for ridx, _ in enumerate(split_results):
        times, aucs = [], []
        for split_idx, (train_idxs, test_idxs) in enumerate(cv_split_idxs):
            split_result = split_results[ridx][split_idx]
            if split_result is None:
                continue
            time_test_idxs = test_idxs[
                (y[test_idxs][y_time] >= np.min(y[train_idxs][y_time]))
                & (y[test_idxs][y_time] <= np.max(y[train_idxs][y_time]))]
            y_pred_idxs = np.where(np.isin(test_idxs, time_test_idxs,
                                           assume_unique=True))[0]
            y_pred = split_result['scores']['te']['y_pred'][y_pred_idxs]
            time = np.arange(np.min(y[time_test_idxs][y_time]),
                             np.max(y[time_test_idxs][y_time]),
                             time_interval_days)
            auc = cumulative_dynamic_auc(y[train_idxs], y[time_test_idxs],
                                         y_pred, time)[0]
    ...
    ax.set_ylabel('Time-dependent AUC', fontsize=axis_fontsize, labelpad=5)
