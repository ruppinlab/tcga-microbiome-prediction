...
print('Running LogisticRegression models')
...
all_roc_scores_df.to_csv('{}/lgr_clinical_model_scores.tsv'
                         .format(out_dir), sep='\t')

dump(all_roc_scores_df,
     '{}/lgr_clinical_model_scores.pkl'.format(out_dir))

r_base.saveRDS(all_roc_scores_df,
               '{}/lgr_clinical_model_scores.rds'.format(out_dir))

mean_scores_df = pd.DataFrame(mean_scores, columns=[
    'Analysis', 'Cancer', 'Target', 'Data Type', 'Mean Score'])
mean_scores_df.to_csv('{}/lgr_clinical_model_mean_scores.tsv'
