...
parser.add_argument("--out-dir", type=str, default="figures/roc_pr", help="out dir")
parser.add_argument(
    "--model-code",
    type=str,
    nargs="+",
        ...
        if m := re.search(split_results_regex, filename):
            ...
            if data_type in ("star", "kraken"):
                ...
                split_results.append(
                    load(
                        "{}/resp/{name}/{name}_split_results.pkl".format(
                            model_results_dir, name=clinical_model_name
                        )
                    )
                )
            else:
            ...
            for ridx, _ in enumerate(split_results):
                ...
                ax.plot(
                    ...
                    color=colors[ridx],
                    label=(
                        "AUROC = {:.2f}".format(np.mean(roc_scores))
                        if ridx == 0
                        else None
                    ),
                    lw=3,
            ...
            ax.set_xlabel("False positive rate", fontsize=axis_fontsize, labelpad=5)
            ax.set_ylabel("True positive rate", fontsize=axis_fontsize, labelpad=5)
            ...
            for fmt in args.file_format:
                fig.savefig(
                    "{}/{}_roc_auc.{}".format(args.out_dir, model_name, fmt),
                    format=fmt,
                    bbox_inches="tight",
                    # matplotlib GH#15497
            ...
            for ridx, _ in enumerate(split_results):
                ...
                ax.step(
                    ...
                    color=colors[ridx],
                    label=(
                        "AUPRC = {:.2f}".format(np.mean(pr_scores))
                        if ridx == 0
                        else None
                    ),
                    lw=3,
            ...
            for fmt in args.file_format:
                fig.savefig(
                    "{}/{}_pr_auc.{}".format(args.out_dir, model_name, fmt),
                    format=fmt,
                    bbox_inches="tight",
                    # matplotlib GH#15497
                    dpi="figure" if fmt == "pdf" else fig_dpi,
            ...
            param_cv_scores = load(
                "{}/resp/{name}/{name}_param_cv_scores.pkl".format(
                    model_results_dir, name=model_name
                )
            )
            ...
            for param in param_cv_scores:
                ...
                param_parts_start_idx = [
                    ...
                    if pipe_step_type_regex.match(p)
                ][-1]
                ...
                if param_parts[-1] in ("k", "n_features_to_select"):
                    ...
                    ax.get_xaxis().set_minor_locator(
                        ticker.LogLocator(base=10, subs="all", numticks=8)
                    )
                    ax.get_xaxis().set_major_locator(
                        ticker.LogLocator(base=10, numticks=len(x_axis))
                    )
                ...
                for metric_idx, metric in enumerate(metrics):
                    ...
                    for idx, param_value_scores in enumerate(param_metric_scores):
                        if metric_idx == 0:
                    ...
                    zorder = 2.5 if metric_idx == 0 else 2.2 if metric_idx == 1 else 2
                    ax.plot(
                        x_axis,
                        mean_cv_scores,
                ...
                pd.DataFrame(tsv_scores).to_csv(
                    "{}/{}_{}_vs_score.tsv".format(args.out_dir, model_name, param_ext),
                    index=False,
                    sep="\t",
                )
